...
<p>Click on the order you want to update. The login listed below is the one they used when placing the order. Instructions for updating an order are available <a href="instructions.html" target="_blank">here</a>.</p>
<p>To create an order for a person, click <a href="http://people.clemson.edu/~siam/shopping/admin_create_order.php"  target="_blank">here</a>.</p>
<div>
...
{
	// list all orders
	$query="SELECT order_id, cu_login, filled_by, delivered, IF(UNIX_TIMESTAMP(date_last_updated) = 0,'never',DATE_FORMAT(date_last_updated, '%c/%e at %l:%i %p')) AS date_last_updated, DATE_FORMAT(date_ordered, '%c/%e/%y') AS date_ordered FROM shopping_orders ORDER BY cu_login";
}
else if(isset($_POST['display']) && $_POST['display'] == "cancelled")
{
	// list cancelled orders
	$query="SELECT order_id, cu_login, filled_by, delivered, IF(UNIX_TIMESTAMP(date_last_updated) = 0,'never',DATE_FORMAT(date_last_updated, '%c/%e at %l:%i %p')) AS date_last_updated, DATE_FORMAT(date_ordered, '%c/%e/%y') AS date_ordered FROM shopping_orders WHERE cancelled = 1 ORDER BY cu_login";
}
else if(isset($_POST['display']) && $_POST['display'] == "filled")
{
	// list filled orders
	$query="SELECT order_id, cu_login, filled_by, delivered, IF(UNIX_TIMESTAMP(date_last_updated) = 0,'never',DATE_FORMAT(date_last_updated, '%c/%e at %l:%i %p')) as date_last_updated, DATE_FORMAT(date_ordered, '%c/%e/%y') as date_ordered From shopping_orders WHERE delivered = 1 AND cancelled = 0 ORDER BY cu_login";
}
else
{
	// list unfilled orders
	$query="SELECT order_id, cu_login, filled_by, delivered, IF(UNIX_TIMESTAMP(date_last_updated) = 0,'never',DATE_FORMAT(date_last_updated, '%c/%e at %l:%i %p')) AS date_last_updated, DATE_FORMAT(date_ordered, '%c/%e/%y') AS date_ordered FROM shopping_orders WHERE delivered = 0 AND cancelled = 0 ORDER BY cu_login";
}
